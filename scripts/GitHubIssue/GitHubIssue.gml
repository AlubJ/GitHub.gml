...
function GitHubIssue(_title = undefined, _body = undefined, _assignee = undefined, _milestone = undefined, _labels = undefined, _assignees = undefined, _type = undefined, _state = undefined, _stateReason = undefined) constructor
{
	...
	static generateJSON = function()
	{
		...
		// Title
		if (title != undefined) _struct[$ "title"] = title;
		
		// Body
		if (body != undefined) _struct[$ "body"] = body;
		
		// Assignee
		if (assignee != undefined) _struct[$ "assignee"] = assignee;
		
		// Milestone
		if (milestone != undefined) _struct[$ "milestone"] = milestone;
		
		// Labels
		if (labels != undefined) _struct[$ "labels"] = labels;
		
		// Assignees
		if (assignees != undefined) _struct[$ "assignees"] = assignees;
		
		// Type
		if (type != undefined) _struct[$ "type"] = type;
		
		// State
		if (state != undefined) _struct[$ "state"] = state;
		
		// State reason
		if (stateReason != undefined) _struct[$ "state_reason"] = stateReason;
