...
function GitHubGist(_description = undefined, _public = undefined) constructor
{
	...
	static generateJSON = function()
	{
		...
		// Desc
		if (description != undefined) _struct[$ "description"] = description;
		
		// Public
		if (public != undefined) _struct[$ "public"] = public ? "true" : "false";
		
		// Files
		if (variable_struct_names_count(files) > 0) _struct[$ "files"] = files;
		else __GitHubError("GitHubGist requires files to be able to be uploaded.");
	...
	{
		var _contentStruct = { content: _content };
		if (_newFilename != undefined) _contentStruct[$ "filename"] = _newFilename;
	...
	{
		if (variable_struct_exists(files, _filename)) variable_struct_remove(files, _filename);
	}
}